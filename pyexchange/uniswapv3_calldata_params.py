...
class IncreaseLiquidityParams(Params):
    ...
    def __init__(self, web3: Web3, token_id: int, amount_0_desired: int, amount_1_desired: int, amount_0_min: int, amount_1_min: int, deadline: int):
        assert (isinstance(web3, Web3))
        assert (isinstance(token_id, int))
...
class ExactInputSingleParams(Params):
    ...
    def __init__(self, web3: Web3, token_in: Token, token_out: Token, fee: int, recipient: Address, deadline: int, amount_in: int, amount_out_minimum: int, sqrt_price_limit_x96: int):
        assert (isinstance(web3, Web3))
...
class ExactOutputParams(Params):
    ...
    def __init__(self, web3: Web3, path: bytes, recipient: Address, deadline: int, amount_out: int, amount_in_maximum: int):
        assert (isinstance(web3, Web3))
        assert (isinstance(path, bytes))
        assert (isinstance(recipient, Address))
        assert(isinstance(deadline, int) or (deadline is None))
